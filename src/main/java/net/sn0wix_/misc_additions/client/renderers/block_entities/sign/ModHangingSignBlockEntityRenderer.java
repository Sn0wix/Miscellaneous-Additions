...
public class ModHangingSignBlockEntityRenderer extends ModSignBlockEntityRenderer<ModHangingSignBlockEntity> {
    public static final EntityModelLayer LAYER_LOCATION = new EntityModelLayer(new Identifier(MiscAdditions.MOD_ID, "hanging_sign/azalea"), "main");
    ...
    public ModHangingSignBlockEntityRenderer(BlockEntityRendererFactory.Context context) {
        super(context);
        MODELS = ImmutableMap.of(ModWoodTypes.AZALEA, new HangingSignModel(context.getLayerModelPart(LAYER_LOCATION)));
    }
    ...
    @Override
    public void render(ModHangingSignBlockEntity signBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
        BlockState blockState = signBlockEntity.getCachedState();
        AbstractSignBlock abstractSignBlock = (AbstractSignBlock)blockState.getBlock();
        HangingSignModel hangingSignModel = this.MODELS.get(ModWoodTypes.AZALEA);
        hangingSignModel.updateVisibleParts(blockState);
        this.render(signBlockEntity, matrixStack, vertexConsumerProvider, i, j, blockState, abstractSignBlock, ModWoodTypes.AZALEA, hangingSignModel);
    }
    ...
    SpriteIdentifier getTextureId(WoodType signType) {
        return new SpriteIdentifier(TexturedRenderLayers.SIGNS_ATLAS_TEXTURE, new Identifier(MiscAdditions.MOD_ID,"entity/signs/hanging/azalea"));
    }
