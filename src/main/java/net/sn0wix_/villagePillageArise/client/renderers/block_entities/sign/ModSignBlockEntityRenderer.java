package net.sn0wix_.villagePillageArise.client.renderers.block_entities.sign;
...
import net.sn0wix_.villagePillageArise.common.block.entities.ModSignBlockEntity;
import net.sn0wix_.villagePillageArise.common.util.ModWoodTypes;
...
public class ModSignBlockEntityRenderer<T extends ModSignBlockEntity> implements BlockEntityRenderer<T> {
    public static final EntityModelLayer LAYER_LOCATION = new EntityModelLayer(new Identifier(VillagePillageAriseMain.MOD_ID, "sign/azalea"), "main");
    ...
    public ModSignBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
        ...
        typeToModel = ImmutableMap.of(ModWoodTypes.AZALEA, new ModSignBlockEntityRenderer.SignModel(ctx.getLayerModelPart(LAYER_LOCATION)));
    }

    @Override
    public void render(T signBlockEntity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        BlockState blockState = signBlockEntity.getCachedState();
        AbstractSignBlock abstractSignBlock = (AbstractSignBlock)blockState.getBlock();
        ModSignBlockEntityRenderer.SignModel signModel = typeToModel.get(ModWoodTypes.AZALEA);
        signModel.stick.visible = blockState.getBlock() instanceof SignBlock;
        this.render(signBlockEntity, matrices, vertexConsumers, light, overlay, blockState, abstractSignBlock, ModWoodTypes.AZALEA, signModel);
    }
    ...
    void render(T entity, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay, BlockState state, AbstractSignBlock block, WoodType woodType, Model model) {
        matrices.push();
    ...
    void renderText(BlockPos pos, SignText signText, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int lineHeight, int lineWidth, boolean front) {
        ...
        int i = ModSignBlockEntityRenderer.getColor(signText);
        ...
        if (signText.isGlowing()) {
            ...
            bl = ModSignBlockEntityRenderer.shouldRender(pos, k);
            l = 0xF000F0;
        } else {
            k = i;
            bl = false;
    ...
    public static final class SignModel
        ...
        public SignModel(ModelPart root) {
            ...
            this.root = root;
            this.stick = root.getChild(ModSignBlockEntityRenderer.STICK);
        }
